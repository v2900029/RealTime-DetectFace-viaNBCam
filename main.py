...
def open_camera():
    ...
    while(True):
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.putText(frame, wh_text, (x+w, y), cv2.FONT_HERSHEY_COMPLEX_SMALL,  1, (0, 255, 255), 1, cv2.LINE_AA)

        # Calculate frames per second
        fps  = np.int(fps_idx / (time.time() - start_time))
        fps_text = 'FPS: {0}'.format(fps)
        cv2.putText(frame, fps_text, (10, 20), cv2.FONT_HERSHEY_COMPLEX_SMALL,  1, (0, 255, 255), 1, cv2.LINE_AA)
